...
hidden_layer_sizes_values = Integer(low=50, high=1500, name="hidden_layer_sizes")
activation_values = Categorical(["identity", "logistic", "tanh", "relu"],name="activation")
learning_rate_init_values = Real(low=1e-8, high=1e-3, name="learning_rate_init")
max_iter_values = Integer(low=100, high=400, name="max_iter")
alpha_values = Real(low=1e-9, high=1e-3, name="alpha")

param_grid = [hidden_layer_sizes_values, activation_values, learning_rate_init_values, max_iter_values, alpha_values]
...
def call_model(hidden_layer_sizes, activation, learning_rate_init, max_iter, alpha):
    global X_train, y_train, X_test, y_test, X_val, y_val
    global best_model, best_mae, best_test_pred, best_param_list

    regr = MLPRegressor(learning_rate="adaptive",
    ...
    if val_mae < best_mae:
        ...
        best_param_list = [["hidden_layer_sizes", hidden_layer_sizes],
                           ["activation", activation],
                           ["learning_rate_init",learning_rate_init],
                           ["max_iter",max_iter],
                           ["alpha",alpha]]
